...
def simulate(grid, start_row, start_col, start_dir, n, visited):
    """
    구슬의 이동을 시뮬레이션하는 함수
    - grid: 격자 정보
    ...
    while 0 <= row < n and 0 <= col < n:  # 격자 안에 있는 동안 반복
        visited.add((row, col, direction))  # 현재 상태 방문 기록
        time += 1
...
def find_max_time(n, grid):
    ...
    for i in range(n):
        max_time = max(max_time, simulate(grid, i, 0, 0, n, set()))  # 왼쪽에서 오른쪽
        max_time = max(max_time, simulate(grid, i, n-1, 2, n, set()))  # 오른쪽에서 왼쪽
        max_time = max(max_time, simulate(grid, 0, i, 1, n, set()))  # 위쪽에서 아래쪽
        max_time = max(max_time, simulate(grid, n-1, i, 3, n, set()))  # 아래쪽에서 위쪽
